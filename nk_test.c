...
void nk_test__run_fixture(
        const struct nk_testsuite__test * tests,
        void (* setup)(void),
        void (* teardown)(void),
        const char * name)
{
    ...
    for (uint32_t i = 0u;
        (tests[i].test_fn != NULL) && (tests[i].name != NULL);
        i++) {
        if (g_testsuite__context.config.is_verbose) {
...
bool np_testsuite__actual(uint32_t line, union np_testsuite__test_val actual)
{
    switch (g_testsuite__context.test_case.type) {
        ...
        case NP_TESTSUITE__TYPE_STR:
            if (strcmp(actual.str,
                g_testsuite__context.test_case.expected.str) != 0) {
                g_testsuite__context.has_failed = true;
            }
            break;
